<script lang="ts">
    ...
    // Set Project title for Nav if a Project is Selected
    let currentProjectTitle: string | null = null
    let tokens = []
    $: tokens = $page.url.pathname.split('/');
    $: if (tokens.length > 2) {
        let thisProject = $Projects.find((p) => p.slug.includes(tokens[tokens.length - 1]))
        if (thisProject) {
            currentProjectTitle = thisProject.brand
        }
    } else {
...
<header class="relative isolate z-10 bg-white">
    <nav class="mx-auto flex max-w-5xl items-center justify-between pt-6 px-6 lg:px-8" aria-label="Global">
        ...
        <div class="flex lg:hidden">
            <button on:click={() => MainMenuOpen = !MainMenuOpen} type="button"
        ...
        <div class="hidden lg:flex lg:gap-x-12">
            <div>
                <button on:click={() => WorkOpen = !WorkOpen} type="button"
                        class="flex items-center gap-x-1 nav-link leading-6 " aria-expanded="false">
                    Work
                    ...
                    {#if currentProjectTitle}
                        <div>
                            <span class="text-sm">{currentProjectTitle}</span>
                        </div>
                    {/if}
                </button>
                <!--
        ...
        <div class="hidden lg:flex lg:flex-1 lg:justify-end">
            <button on:click={Contact.toggle}
                    class="leading-6 text-2xl hover:bg-blue-600 font-light text-white flex-4 bg-blue-800 rounded px-3 py-2 hidden md:inline-block">
                Contact &rarr;<span aria-hidden="true"></span></button>
        </div>
    </nav>
                ...
                <div class="mt-6 flow-root">
                    <div class="-my-6 divide-y divide-gray-500/10">
                        <div class="space-y-2 py-6">
                            <div class="-mx-3">
                                <button on:click={() => WorkMobileOpen = !WorkMobileOpen} type="button"
                                    ...
                                    -->
                                    <svg class="{WorkMobileOpen ? 'rotate-180' : ''} h-5 w-5 flex-none"
